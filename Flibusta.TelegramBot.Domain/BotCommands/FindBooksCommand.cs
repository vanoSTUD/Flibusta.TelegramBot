...
public class FindBooksCommand : CommandBase
{
    ...
    /// <param name="args">В качестве callback данных: [pageNumber]</param>
    ...
    public override async Task ExecuteAsync(Update update, string[]? args = null, CancellationToken cancellationToken = default)
    {
        ...
        catch (RequestException ex)
        {
            ...
            await _bot.SendTextMessageAsync(message.Chat, "Не удалось найти книги", parseMode: ParseMode.Html, cancellationToken: cancellationToken);
            return;
        }
        ...
        {
            _logger.LogError("Исключение при работе FindBooksCommand.ExecuteAsync(): {ex}", ex);

            await _bot.SendTextMessageAsync(message.Chat, "Не удалось найти книги по названию", parseMode: ParseMode.Html, cancellationToken: cancellationToken);
        }
    }
}
